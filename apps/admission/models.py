...
class OnlineApplication(BaseModel):
    ...
    previous_school = models.CharField(max_length=200, verbose_name=_("Previous School/College"))
    previous_qualification = models.CharField(max_length=100, verbose_name=_("Previous Class/Grade"))
    ...
    def clean(self):
        ...
        # Age validation for school
        if self.age < 3:
            errors['date_of_birth'] = _("Applicant must be at least 3 years old for school admission")
            
        if self.age > 18:
            errors['date_of_birth'] = _("Applicant age seems too high for school admission")
            
        # Email uniqueness in same admission cycle
        if OnlineApplication.objects.filter(
            email=self.email,
            admission_cycle=self.admission_cycle,
            tenant=self.tenant
        ).exclude(id=self.id).exists():
            errors['email'] = _("An application with this email already exists in this admission cycle")
            
        # Eligibility check
        if self.status != "DRAFT" and not self.is_eligible:
            errors['date_of_birth'] = _("Does not meet age eligibility criteria for this class")
...
def application_document_upload_path(instance, filename):
    ...
    filename = f"{slugify(instance.application.application_number)}_{slugify(instance.document_type)}_{uuid.uuid4().hex[:8]}.{ext}"
    return os.path.join(
        "admission_documents",

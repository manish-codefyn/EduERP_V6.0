...
class FeeStructureCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    model = FeeStructure
    fields = ['name', 'academic_year', 'class_name', 'fee_type', 'frequency', 'amount', 
              'due_day', 'late_fee_applicable', 'late_fee_amount', 'late_fee_after_days', 
...
class FeeDiscountCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    model = FeeDiscount
    fields = ['name', 'code', 'description', 'discount_type', 'value', 'applicable_to', 
              'max_discount_amount', 'max_usage_per_student', 'total_usage_limit', 
...
class InvoiceCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    model = Invoice
    fields = ['student', 'academic_year', 'billing_period', 'issue_date', 'due_date', 
              'payment_terms', 'notes']
...
class PaymentCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    model = Payment
    fields = ['invoice', 'amount', 'payment_date', 'payment_method', 'reference_number', 
              'bank_name', 'transaction_id', 'notes']

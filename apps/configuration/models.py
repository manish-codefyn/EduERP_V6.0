...
class SystemSetting(BaseModel):
    ...
    def clean(self):
        ...
        if not self.key.isidentifier():
            errors['key'] = _('Key must be a valid identifier (letters, numbers, underscores)')
        
        # Value validation based on type
        if self.is_required and self.value is None:
            errors['value_string'] = _('Value is required for this setting')
        
        # Regex validation
        if self.validation_regex and self.value_string:
            import re
            if not re.match(self.validation_regex, self.value_string):
                errors['value_string'] = self.validation_message or _('Value does not match required pattern')
        
        # Range validation
        if self.min_value is not None and self.value is not None:
            if isinstance(self.value, (int, float, Decimal)) and self.value < self.min_value:
                errors['value_decimal'] = _('Value must be at least {}').format(self.min_value)
        
        if self.max_value is not None and self.value is not None:
            if isinstance(self.value, (int, float, Decimal)) and self.value > self.max_value:
                errors['value_decimal'] = _('Value must be at most {}').format(self.max_value)
        
        # Choice validation
        if self.setting_type in ['CHOICE', 'MULTI_CHOICE'] and self.choices:
            if self.setting_type == 'CHOICE' and self.value_string not in self.choices:
                errors['value_string'] = _('Value must be one of: {}').format(', '.join(self.choices))
            elif self.setting_type == 'MULTI_CHOICE' and self.value_json:
                if not all(choice in self.choices for choice in self.value_json):
                    errors['value_json'] = _('All values must be from: {}').format(', '.join(self.choices))
